...
me.skills = ["Javascript", "Typescript", "Python", "Java"];
console.log(me);
...
console.log(splitMe("Ciao sono davide"));
...
console.log(onlyLetters("C2i302ao25   78r"));
...
console.log(isThisAnEmail("davide.faggionato@gmail.com"));
...
const howManyDays = (date) => {
  ...
  const milliseconds = today - initDate;
  const daysSince = milliseconds / (1000 * 3600 * 24);
  return Math.floor(daysSince);
};

console.log(howManyDays("2/1/2024"));
...
console.log(isTodayMyBirthday(9, 2));
...
console.log(deleteProp({ name: "Davide", surname: "Faggionato" }, "surname"));
...
const newestMovie = (arr) => {
  ...
  for (let movie of movies) {
    if (Number(movie.Year) > Number(newest.Year)) {
      newest = movie;
    }
  }
  return newest;
...
const onlyTheYears = (arr) => {
  const res = arr.map((movie) => movie.Year);
  return res;
};
...
const onlyInLastMillennium = (arr) => {
  const res = arr.filter((movie) => Number(movie.Year) < 2001);
  return res;
};
...
const searchByTitle = (str) => {
  ...
  movies.forEach((movie) => {
    if (movie.Title.includes(str)) {
      moviesWithStr.push(movie);
    }
  });
  return moviesWithStr;
};
...
const removeIndex = (num) => {
  const newArr = movies.splice(num, 1);
  return movies;
};
